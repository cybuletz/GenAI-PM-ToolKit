...
def link_response_sheet(creds, drive_service, form_id, title):
    """
    Create a Google Sheet to collect form responses.

    Uses creds directly (not extracted from a service object) to build
    the Sheets API client — extracting credentials from service._http
    is unreliable in newer versions of google-api-python-client.
